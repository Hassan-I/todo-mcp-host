...
class McpClient:
    ...
    def list_tools(self) -> list[dict]:
        ...
        try:
            ...
            # The MCP SDK runs the HTTP transport inside an anyio TaskGroup, so a
            # connection failure arrives wrapped in an ExceptionGroup. except*
            # reaches into the group to catch the nested ConnectError.
            raise ConnectionError("MCP server unreachable") from eg
    ...
    def call_tool(self, name: str, arguments: dict) -> dict:
        ...
        try:
            ...
            # See list_tools: connection failures are wrapped in an ExceptionGroup
            # by the MCP SDK's anyio TaskGroup, so we catch with except*.
            raise ConnectionError("MCP server unreachable") from eg
